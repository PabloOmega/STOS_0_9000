...
lv_obj_t * ta;	//text area

void screen_init(void){
	SSD1963_Init();
	SSD1963_DrawTestScreen();
	lv_init();

	lv_disp_t * disp = lv_disp_create(480, 272); /*Basic initialization with horizontal and vertical resolution in pixels*/
	lv_disp_set_flush_cb(disp, flush_cb); /*Set a flush callback to draw to the display*/
...
void LVGLTimer(void *argument)
{
  ...
  for(;;)
  {
	  osMutexAcquire(lvglMutexHandle, portMAX_DELAY);	//Safe critical section
